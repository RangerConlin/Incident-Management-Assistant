...
def test_is_collection_complete_true_until_something_is_dropped() -> None:
    ...
    incident_cache.load_snapshot(
        "INC-COMPLETE",
        {"checkin_history": [{"_id": "h-1", "person_record": 1, "ts": "2026-07-07T00:00:00+00:00"}]},
        meta={"policy": {"max_heavy_collection_docs": 5, "heavy_collections": ["checkin_history"]}},
    )

    assert incident_cache.is_collection_complete("checkin_history") is True
    ...
    assert incident_cache.is_collection_complete("checkin_history") is False
...
def test_is_collection_complete_reflects_server_reported_truncation() -> None:
    ...
    incident_cache.load_snapshot(
        ...
        {"checkin_history": []},
        meta={"truncated": {"checkin_history": {"loaded": 0, "total": 900, "reason": "snapshot byte budget"}}},
    )
