...
| `modules/operations` teams | Done | `get_team` (Team Detail, ICS-211, org chart), the taskings team picker `list_all_teams`, `get_checked_in_teams`/`get_unchecked_teams` (ICS-211 check-in/disband team lists, filtering cached `teams` by the same `DERIVED_CHECKED_IN_STATUSES` set and `disbanded` flag the server uses), and the Team Status Board's `list_tasks_for_assignment` (`modules/operations/data/repository.py`, backs the right-click "assign & set status" submenus in `team_status_panel.py`) all read the cached `teams`/`tasks` collections first, falling back to their API GET on cache miss/no active incident. Team assignment history covered via taskings' cached `task_teams`/audit reads (embedded in `tasks` docs). |
