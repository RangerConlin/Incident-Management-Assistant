...
def _find_person(personnel_id: Any) -> dict[str, Any] | None:
    if personnel_id is None:
        return None
    value = str(personnel_id).strip()
    if not value:
        return None
    return (
        _personnel_col().find_one({"person_id": value})
        or _personnel_col().find_one({"id": value})
        or (_personnel_col().find_one({"int_id": int(value)}) if value.isdigit() else None)
    )


def _resolve_personnel_id(body: dict[str, Any], existing_user: dict[str, Any] | None = None) -> str | None:
    explicit = body.get("personnel_id") or (existing_user or {}).get("personnel_id")
    person = _find_person(explicit)
    if person:
        return str(person.get("person_id") or person.get("id") or person.get("int_id"))

    for candidate in (body.get("username"), body.get("user_id"), body.get("badge_number")):
        person = _find_person(candidate)
        if person:
            return str(person.get("person_id") or person.get("id") or person.get("int_id"))
    return str(explicit).strip() if explicit else None
...
def _session_response(session: dict[str, Any]) -> dict[str, Any]:
    ...
    person = _find_person(doc.get("personnel_id") or (user or {}).get("personnel_id"))
...
def start_session(body: dict[str, Any] = Body(...)) -> dict[str, Any]:
    ...
    personnel_id = _resolve_personnel_id(body, existing_user)
    user_doc = {
        "user_id": user_id,
        "username": username,
