...
isn't the source, a device is trusted for whichever team_id it sends (no
roster-membership check), an unknown team_id or an unchecked-in person's
ping is silently ignored, stop only clears when the stopper is the current
source, the desktop-facing team-locations GET, and the server-side
auto-clear on demobilization.
"""
...
LEADER_ID = 100
MEMBER_ID = 101
OUTSIDER_ID = 102  # registered + checked in, not listed in the team's roster
TEAM_INT_ID = 1
...
def _seed():
    _teams_col().insert_one({
        ...
        # Deliberately does NOT include OUTSIDER_ID — team attribution comes
        # from the ping's team_id, not this roster list.
...
def test_leader_ping_overwrites():
    ...
    with TestClient(app) as client:
        res = client.post("/api/mobile/location", json={"token": TOKEN_LEADER, "lat": 1.0, "lon": 2.0, "team_id": TEAM_INT_ID})
...
def test_non_leader_ping_ignored_while_leader_is_source():
    ...
    app = create_app()
    with TestClient(app) as client:
        client.post("/api/mobile/location", json={"token": TOKEN_LEADER, "lat": 1.0, "lon": 2.0, "team_id": TEAM_INT_ID})
        res = client.post("/api/mobile/location", json={"token": TOKEN_MEMBER, "lat": 9.0, "lon": 9.0, "team_id": TEAM_INT_ID})
    assert res.status_code == 200
...
def test_non_leader_ping_accepted_once_leader_not_source():
    ...
    app = create_app()
    with TestClient(app) as client:
        res = client.post("/api/mobile/location", json={"token": TOKEN_MEMBER, "lat": 3.0, "lon": 4.0, "team_id": TEAM_INT_ID})
    assert res.status_code == 200
    assert res.json() == {"ok": True, "recorded": True}
    team = _teams_col().find_one({"int_id": TEAM_INT_ID})
...
def test_person_not_on_roster_still_recorded_for_signed_in_team():
    """The device's signed-in team_id is trusted directly — no roster-
    membership gate. OUTSIDER_ID is checked in but not in the team's
    members_json, and the ping still records."""
    _clear()
    _seed()
    app = create_app()
    with TestClient(app) as client:
        res = client.post("/api/mobile/location", json={"token": TOKEN_OUTSIDER, "lat": 7.0, "lon": 8.0, "team_id": TEAM_INT_ID})
    assert res.status_code == 200
    assert res.json() == {"ok": True, "recorded": True}
    team = _teams_col().find_one({"int_id": TEAM_INT_ID})
    assert team["current_location_lat"] == 7.0
    assert team["current_location_person_record"] == OUTSIDER_ID
    _clear()
...
def test_unchecked_in_person_ping_silently_ignored():
    ...
    app = create_app()
    with TestClient(app) as client:
        res = client.post("/api/mobile/location", json={"token": TOKEN_UNCHECKED, "lat": 5.0, "lon": 6.0, "team_id": TEAM_INT_ID})
    assert res.status_code == 200
    assert res.json() == {"ok": True, "recorded": False}
    team = _teams_col().find_one({"int_id": TEAM_INT_ID})
...
def test_unknown_team_id_silently_ignored():
    _clear()
    _seed()
    app = create_app()
    with TestClient(app) as client:
        res = client.post("/api/mobile/location", json={"token": TOKEN_LEADER, "lat": 5.0, "lon": 6.0, "team_id": 999})
    assert res.status_code == 200
    assert res.json() == {"ok": True, "recorded": False}
    team = _teams_col().find_one({"int_id": TEAM_INT_ID})
    assert team["current_location_lat"] is None
...
def test_missing_team_id_is_a_bad_request():
    _clear()
    _seed()
    app = create_app()
    with TestClient(app) as client:
        res = client.post("/api/mobile/location", json={"token": TOKEN_LEADER, "lat": 5.0, "lon": 6.0})
...
def test_stop_clears_only_when_stopper_is_current_source():
    ...
    app = create_app()
    with TestClient(app) as client:
        client.post("/api/mobile/location", json={"token": TOKEN_MEMBER, "lat": 3.0, "lon": 4.0, "team_id": TEAM_INT_ID})
...
def test_get_team_locations_returns_active_dot():
    ...
    rows = res.json()
    assert len(rows) == 1
    assert rows[0]["team_id"] == TEAM_INT_ID
