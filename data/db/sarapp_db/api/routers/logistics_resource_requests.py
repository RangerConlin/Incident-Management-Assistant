...
def _col(incident_id: str):
    return get_db(f"sarapp_incident_{incident_id}")[IncidentCollections.LOGISTICS_RESOURCE_REQUESTS]
...
def _fetch(col, request_id: str) -> dict:
    doc = col.find_one({"id": request_id})
    if not doc:
...
def list_requests(
    ...
    docs = list(col.find(query, {"items": 0, "approvals": 0, "fulfillments": 0, "audit": 0},
                         sort=[("created_utc", -1)]))
...
def update_request(incident_id: str, request_id: str, body: dict[str, Any]) -> dict:
    ...
    now = _now()
    body["last_updated_utc"] = now
    if doc.get("status", "DRAFT") != "DRAFT":
        body["version"] = doc.get("version", 1) + 1
    audit_entry = {"event": "update", "ts_utc": now, "actor_id": actor_id, "fields": list(body.keys())}
    result = col.find_one_and_update(
        {"id": request_id},
        {"$set": body, "$push": {"audit": audit_entry}},
...
def add_items(incident_id: str, request_id: str, body: list[dict[str, Any]]) -> dict:
    ...
    col.update_one({"id": request_id}, {"$push": {"items": {"$each": items}}})
...
def replace_items(incident_id: str, request_id: str, body: list[dict[str, Any]]) -> dict:
    ...
    _fetch(col, request_id)
    items = [{"id": item.get("id") or _new_id(), **item} for item in body]
    col.update_one({"id": request_id}, {"$set": {"items": items, "last_updated_utc": _now()}})
    return {"ids": [item["id"] for item in items]}
