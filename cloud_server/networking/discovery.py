...
Cloud servers typically don't broadcast UDP, but this module is included so
the cloud server shares the same networking interface as the LAN server.
DiscoveryBroadcaster can be disabled via discovery_enabled=False at startup.
"""
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    logger.warning("Failed to broadcast discovery packet: %s", exc)
...
class DiscoveryClient:
    """Listens for SARApp Server UDP announcements."""
    ...
    def manual_server(host: str, port: int, *, name: str = "Manual SARApp Server") -> ServerInfo:
        return ServerInfo(server_id=f"manual-{host}-{port}", server_name=name, host=host, port=port)
