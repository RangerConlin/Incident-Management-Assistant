...
for the duration of each Safety Risk Manager test, so modules.safety.orm.service
exercises the actual MongoDB-backed router in data/db/sarapp_db/api/routers/safety.py
...
def orm_app_client():
    ...
    Also clears this module's test incidents' hazard collections before and
    after each test so runs are deterministic regardless of prior runs.
    """
    from sarapp_db.api.app import create_app

    _clear_hazard_collections()
    app = create_app()
