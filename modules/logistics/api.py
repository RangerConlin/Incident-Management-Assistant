...
def list_requests(mission_id: str):
    with services.with_mission_session(mission_id) as session:
...
def create_request_endpoint(mission_id: str, data: ResourceRequestCreate):
    req = services.create_request(mission_id, data)
...
def approve_request_endpoint(
    ...
    services.approve_request(mission_id, request_id, data)
    return {"status": "ok"}
...
def assign_request_endpoint(
    ...
    services.assign_request(mission_id, request_id, data)
    return {"status": "ok"}


@router.get("/equipment", response_model=list[EquipmentItemRead])
def list_equipment(mission_id: str):
    with services.with_mission_session(mission_id) as session:
...
def add_equipment_endpoint(mission_id: str, data: EquipmentItemCreate):
    item = services.add_equipment(mission_id, data)
    return EquipmentItemRead.from_orm(item)


@router.post("/equipment/{equipment_id}/checkout")
def checkout_equipment_endpoint(mission_id: str, equipment_id: int, actor_id: int):
    services.checkout_equipment(mission_id, equipment_id, actor_id)
    return {"status": "ok"}


@router.post("/equipment/{equipment_id}/checkin")
def checkin_equipment_endpoint(mission_id: str, equipment_id: int, actor_id: int):
    services.checkin_equipment(mission_id, equipment_id, actor_id)
    return {"status": "ok"}


@router.post("/requests/{request_id}/print/ics213rr")
def print_request(mission_id: str, request_id: int):
    path, _bytes = generate_pdf(mission_id, request_id)
    return {"path": path}
