    ...
    def _show_key_help(self) -> None:
        ...
        QMessageBox.information(
            ...
            (
                ...
                "Start with one of the core data hubs and append nested fields"
                " separated by dots. Common roots you can build from are:\n"
                "  • incident — overall metadata like `incident.number`,"
                " `incident.ic_name`, `incident.op_period`\n"
                "  • planning — planning section details such as"
                " `planning.scribe` or `planning.iap.preparer`\n"
                "  • operations.ics204 — the active assignment, for example"
                " `operations.ics204.team_leader`,"
                " `operations.ics204.assignment`,"
                " `operations.ics204.resources`\n"
                "  • teams.current — whichever team is in focus, e.g."
                " `teams.current.team_id`, `teams.current.leader_name`\n"
                "  • logistics or support catalogues —"
                " `logistics.staging_manager`, `logistics.resources.cache_name`\n"
                "  • communications — radio plan values like"
                " `communications.ics205.primary_channel`\n\n"
                "Each segment should be lowercase, avoid spaces, and remain"
                " stable over time so templates continue to resolve. The"
                " resolver follows those segments against the context supplied"
                " when a form instance is generated.\n\n"
                "If you need something outside the built-in catalogue, craft a"
                " descriptive dotted key that matches the data your workflow"
                " provides, then add it as a custom binding so it becomes"
                " selectable for every template."
            ),
        )
