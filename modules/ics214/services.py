...
def create_stream(data: StreamCreate) -> ICS214Stream:
    with with_mission_session(data.mission_id) as session:
...
def list_streams(mission_id: str) -> List[ICS214Stream]:
    with with_mission_session(mission_id) as session:
        result = session.execute(select(ICS214Stream)).scalars().all()
        return result


def update_stream(mission_id: str, stream_id: str, data: StreamUpdate) -> ICS214Stream | None:
    with with_mission_session(mission_id) as session:
...
def add_entry(mission_id: str, stream_id: str, data: EntryCreate, autogenerated: bool = False, id_hash: str | None = None) -> Dict[str, Any]:
    with with_mission_session(mission_id) as session:
        stream = session.get(ICS214Stream, stream_id)
        if not stream:
...
def list_entries(mission_id: str, stream_id: str) -> List[Dict[str, Any]]:
    with with_mission_session(mission_id) as session:
...
def export_stream(mission_id: str, stream_id: str, options: ExportRequest) -> ICS214Export:
    entries = list_entries(mission_id, stream_id)
    if not options.include_auto:
        entries = [e for e in entries if not e["autogenerated"]]
    base_dir = Path("data") / "uploads" / "ics214" / mission_id
